...
function createLineObject(){
  ...
  line.insertText = function(index, text){
    ...
    {
      charObject = createCharacterObject()
      charObject.value = char
      charObjectArray.push(charObject)
    }

    line.content.splice(index, 0, ...charObjectArray)
  }
  ...
  line.convertToString = function(){
    ...
    for(let charObj of this.content)
    {
      string += charObj.value
    }
    
    return string
  }

  line.addCharacter = function(char, index)
  {

    let charObject = createCharacterObject()
    charObject.value = char; 
    this.content[index] = charObject
...
function createDocumentObject(){
  ...
  doc.convertLineToString = function(lineNumber)
  {
    ...
    return this.getLine(lineNumber).convertToString();
